...
#define hlp BP
/* ---------------------------------------*/
TEXT p256SubInternal4fpga(SB),NOSPLIT,$0
	XORQ mul0, mul0
	SUBQ t0, acc4
	SBBQ t1, acc5
	SBBQ t2, acc6
	...
	RET
/* ---------------------------------------*/
TEXT p256MulInternal4fpga(SB),NOSPLIT,$0
	MOVQ acc4, mul0
	MULQ t0
	MOVQ mul0, acc0
	MOVQ mul1, acc1
	...
	RET
/* ---------------------------------------*/
TEXT p256SqrInternal4fpga(SB),NOSPLIT,$0
...
TEXT ·p256PointAddAffineAsm(SB),0,$512-96
	...
	// Begin point add
	LDacc (z1in)
	CALL p256SqrInternal4fpga(SB)	// z1ˆ2
	ST (z1sqr)

	LDt (x2in)
	CALL p256MulInternal4fpga(SB)	// x2 * z1ˆ2

	LDt (x1in)
	CALL p256SubInternal4fpga(SB)	// h = u2 - u1
	ST (h)

	LDt (z1in)
	CALL p256MulInternal4fpga(SB)	// z3 = h * z1
	ST (zout)

	LDacc (z1sqr)
	CALL p256MulInternal4fpga(SB)	// z1ˆ3

	LDt (y2in)
	CALL p256MulInternal4fpga(SB)	// s2 = y2 * z1ˆ3
	ST (s2)

	LDt (y1in)
	CALL p256SubInternal4fpga(SB)	// r = s2 - s1
	ST (r)

	CALL p256SqrInternal4fpga(SB)	// rsqr = rˆ2
	ST (rsqr)

	LDacc (h)
	CALL p256SqrInternal4fpga(SB)	// hsqr = hˆ2
	ST (hsqr)

	LDt (h)
	CALL p256MulInternal4fpga(SB)	// hcub = hˆ3
	ST (hcub)

	LDt (y1in)
	CALL p256MulInternal4fpga(SB)	// y1 * hˆ3
	ST (s2)

	LDacc (x1in)
	LDt (hsqr)
	CALL p256MulInternal4fpga(SB)	// u1 * hˆ2
	ST (h)

	p256MulBy2Inline			// u1 * hˆ2 * 2, inline
	LDacc (rsqr)
	CALL p256SubInternal4fpga(SB)	// rˆ2 - u1 * hˆ2 * 2

	LDt (hcub)
	CALL p256SubInternal4fpga(SB)
	ST (xout)
...
// p256IsZero4fpga returns 1 in AX if [acc4..acc7] represents zero and zero
// otherwise. It writes to [acc4..acc7], t0 and t1.
TEXT p256IsZero4fpga(SB),NOSPLIT,$0
...
TEXT ·p256PointAddAsm(SB),0,$680-80
	...
	// Begin point add
	LDacc (z2in)
	CALL p256SqrInternal4fpga(SB)	// z2ˆ2
	ST (z2sqr)
	LDt (z2in)
	CALL p256MulInternal4fpga(SB)	// z2ˆ3
	LDt (y1in)
	CALL p256MulInternal4fpga(SB)	// s1 = z2ˆ3*y1
	ST (s1)
	...
	CALL p256MulInternal4fpga(SB)	// z1ˆ3
	LDt (y2in)
	CALL p256MulInternal4fpga(SB)	// s2 = z1ˆ3*y2
	ST (s2)

	LDt (s1)
	CALL p256SubInternal4fpga(SB)	// r = s2 - s1
	ST (r)
	CALL p256IsZero4fpga(SB)
	MOVQ AX, points_eq

	LDacc (z2sqr)
	LDt (x1in)
	CALL p256MulInternal4fpga(SB)	// u1 = x1 * z2ˆ2
	ST (u1)
	LDacc (z1sqr)
	LDt (x2in)
	CALL p256MulInternal4fpga(SB)	// u2 = x2 * z1ˆ2
	ST (u2)
	...
	LDacc (z1in)
	LDt (z2in)
	CALL p256MulInternal4fpga(SB)	// z1 * z2
	LDt (h)
	CALL p256MulInternal4fpga(SB)	// z1 * z2 * h
	ST (zout)

	LDacc (hsqr)
	LDt (u1)
	CALL p256MulInternal4fpga(SB)	// hˆ2 * u1
	ST (u2)
