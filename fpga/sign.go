...
type endorserSignWorker struct {
	...
	cRpcResultChMap map[int] chan<-*pb.BatchReply_SignGenReply

	m          *sync.Mutex
	cSgReqList *list.List
...
func (w *endorserSignWorker) init() {
	...
	w.cRpcResultChMap = make(map[int] chan<-*pb.BatchReply_SignGenReply)
...
func (w *endorserSignWorker) work() {
	...
	go func() {
		for task := range w.taskCh {
			w.m.Lock()
			w.cSgReqList.PushBack(task.in)
			reqId := w.cSgReqList.Len() - 1
			task.in.ReqId = fmt.Sprintf("%064d", reqId)
			w.m.Unlock()
			w.cRpcResultChMap[reqId] = task.out
		}
	}()
	...
	go func() {
		...
		for true {
			...
			// rpc
			w.m.Lock()
			if w.cSgReqList.Len() > 0 {
				size := w.cSgReqList.Len()
				if size > w.batchSize {
					size = w.batchSize
				}
				var sgReqs []*pb.BatchRequest_SignGenRequest
				for i := 0; i < size; i++ {
					element := w.cSgReqList.Front()
					w.cSgReqList.Remove(element)
				...
				if w.cSgReqList.Len() > w.batchSize {
					w.logger.Warningf("current w.cSgReqList.Len is %d, which is bigger than the batch size(%d)", w.cSgReqList.Len(), w.batchSize)
				}
				...
				if err != nil {
					...
					// Attention!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					// attention! the results of rpcRequests and cRpcResultChMap might be not correct.
					...
					w.logger.Errorf("pending w.cSgReqList.Len is %d", w.cSgReqList.Len())
					for k, v := range w.cRpcResultChMap {
...
func (w *endorserSignWorker) parseResponse(response *pb.BatchReply) {
	...
	for _, sig := range signatures {
		...
		if err != nil || w.cRpcResultChMap[reqId] == nil {
			for k, v := range w.cRpcResultChMap {
				w.logger.Errorf("w.cRpcResultChMap[%v]: %v", k, v)
			}
		...
		w.cRpcResultChMap[reqId] <- sig
		close(w.cRpcResultChMap[reqId])
		delete(w.cRpcResultChMap, reqId)
